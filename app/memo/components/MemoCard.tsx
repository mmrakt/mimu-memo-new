...
export default function MemoCard({ post, index }: MemoCardProps) {
  ...
  return (
    <LinkComponent
      {...linkProps}
      className="bg-white/90 dark:bg-slate-800/50 backdrop-blur-sm border border-indigo-500/20 dark:border-indigo-500/10 rounded-2xl overflow-hidden hover:border-indigo-500/30 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-indigo-500/10 cursor-pointer opacity-0 animate-fade-in-up block"
      style={{
    ...
    >
      <div className="relative h-48 bg-gradient-to-br from-indigo-600 to-cyan-600 overflow-hidden group">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-slate-100/60 dark:from-slate-900/60 to-transparent"></div>
      </div>
      <div className="p-6">
        ...
        <h2 className="text-xl font-bold mb-3 text-slate-900 dark:text-slate-100 group-hover:text-indigo-400 transition-colors">
          {post.title}
        </h2>
        {post.excerpt && (
          <p className="text-slate-600 dark:text-slate-400 line-clamp-3 mb-4">{post.excerpt}</p>
        )}
      </div>
    </LinkComponent>
  );
